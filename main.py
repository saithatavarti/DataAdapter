...
@app.get("/{file_name}/{user}/{offset}/{limit}")
def read_file(file_name,user,offset,limit):
    ...
    with open(my_value) as req:
        ...
        if(db_type == "sqlite"):
            ...
            authentication_query = authentication_query + " " + "where name = " + "'" + username + "'" 
            ...
            count = int(count[0])
            # df = pd.read_json (r'E:\sqlite_data_adapter\response_1618566546299.json')
            # df.to_csv (r'E:\sqlite_data_adapter\myfile.csv', index = None)
